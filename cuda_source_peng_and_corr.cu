                                  ...
                                  , double* kx, double* ky, int Height, int Width, int VOL)
{
	...
	if(atom < atom_nums)
	{
		if (row < Height && col < Width)
		{
			float part1 = coff[atom * 13 + 3] * expf((-1) * coff[atom * 13 + 4] * s_2[row * Width + col]);
			float part2 = coff[atom * 13 + 5] * expf((-1) * coff[atom * 13 + 6] * s_2[row * Width + col]);
			float part3 = coff[atom * 13 + 7] * expf((-1) * coff[atom * 13 + 8] * s_2[row * Width + col]);
			float part4 = coff[atom * 13 + 9] * expf((-1) * coff[atom * 13 + 10] * s_2[row * Width + col]);
			float part5 = coff[atom * 13 + 11] * expf((-1) * coff[atom * 13 + 12] * s_2[row * Width + col]);
			float sum = coff[atom * 13 + 2] * (part1 + part2 + part3 + part4 + part5);
			float temp = (-2) * (coff[atom * 13 + 0] * kx[row * Width + col] + coff[atom * 13 + 1] * ky[row * Width + col]
			) * 3.14159265;
                                   ...
                                   , int Height, int Width, int VOL)
{
	...
	if(atom < atomi_nums)
	{
		if (row < Height && col < Width)
		{
			float atom_part1 = coff[atom * 25 + 4] * expf((-1) * coff[atom * 25 + 5] * s_2[row * Width + col]);
			float atom_part2 = coff[atom * 25 + 6] * expf((-1) * coff[atom * 25 + 7] * s_2[row * Width + col]);
			float atom_part3 = coff[atom * 25 + 8] * expf((-1) * coff[atom * 25 + 9] * s_2[row * Width + col]);
			float atom_part4 = coff[atom * 25 + 10] * expf((-1) * coff[atom * 25 + 11] * s_2[row * Width + col]);
			float atom_part5 = coff[atom * 25 + 12] * expf((-1) * coff[atom * 25 + 13] * s_2[row * Width + col]);

			float atom_sum = coff[atom * 25 + 3] * (atom_part1 + atom_part2 + atom_part3 + atom_part4 + atom_part5);

			float ion_part1 = coff[atom * 25 + 15] * expf((-1) * coff[atom * 25 + 16] * s_2[row * Width + col]);
			float ion_part2 = coff[atom * 25 + 17] * expf((-1) * coff[atom * 25 + 18] * s_2[row * Width + col]);
			float ion_part3 = coff[atom * 25 + 19] * expf((-1) * coff[atom * 25 + 20] * s_2[row * Width + col]);
			float ion_part4 = coff[atom * 25 + 21] * expf((-1) * coff[atom * 25 + 22] * s_2[row * Width + col]);
			float ion_part5 = coff[atom * 25 + 23] * expf((-1) * coff[atom * 25 + 24] * s_2[row * Width + col]);

			float ion_sum = coff[atom * 25 + 14] * (ion_part1 + ion_part2 + ion_part3 + ion_part4 + ion_part5);

			float sum = coff[atom * 25 + 2] * (atom_sum + ion_sum);

			float temp = (-2) * (coff[atom * 25 + 0] * kx[row * Width + col] + coff[atom * 25 + 1] * ky[row * Width + col]
			) * 3.14159265;
			cuda_result[row * Width + col].x = cuda_result[row * Width + col].x + cosf(temp) * sum;
			cuda_result[row * Width + col].y = cuda_result[row * Width + col].y + sinf(temp) * sum;
                                ...
                                , int Height, int Width, int VOL)
{
	...
	if(atom < atomi_nums)
	{
		...
		{	
			...
			float part4 = coff_absob[atom * 10 + 6] * expf((-1) * coff_absob[atom * 10 + 7] * s_2[row * Width + col]);
			float part5 = coff_absob[atom * 10 + 8] * expf((-1) * coff_absob[atom * 10 + 9] * s_2[row * Width + col]);
			float sum = coff[atom * 25 + 2] * (part1 + part2 + part3 + part4 + part5);
			float temp = (-2) * (coff[atom * 25 + 0] * kx[row * Width + col] + coff[atom * 25 + 1] * ky[row * Width + col]
			) * 3.14159265;
			cuda_result[row * Width + col].x = cuda_result[row * Width + col].x + (-1) * sinf(temp) * sum;
			cuda_result[row * Width + col].y = cuda_result[row * Width + col].y + cosf(temp) * sum;
...
__global__ void cal_atomORion_fun_corr(cufftComplex* cuda_result, double* cuda_cmi, double parameter,
										float* coff, double* s_2, int atom_nums, 
										double* kx, double* ky, int Height, int Width, int VOL)
{
	int id = (blockIdx.y * gridDim.x + blockIdx.x) * (blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x + threadIdx.x);
	int atom = id / VOL;
	...
	if(atom < atom_nums)
	{
		if (row < Height && col < Width)
		{
			float part1 = coff[atom * 5 + 3];
			int index = (int)coff[atom * 5 + 4] - 1;
			double* cmi_p = cuda_cmi + index * VOL;
			float part2 = (float)cmi_p[row * Width + col];
			float part3 = expf((-1) * coff[atom * 5 + 2] * s_2[row * Width + col]);
			float part4 = (-2) * (coff[atom * 5 + 0] * kx[row * Width + col] + coff[atom * 5 + 1] * ky[row * Width + col]) * 3.14159265;
			float t1 = parameter * cosf(part4) * part1 * part2 * part3;
			float t2 = parameter * sinf(part4) * part1 * part2 * part3;
			cuda_result[row * Width + col].x = cuda_result[row * Width + col].x + t1;
			cuda_result[row * Width + col].y = cuda_result[row * Width + col].y + t2;
		}
	}
}


__global__ void cal_atomORion_fun_i_corr(cufftComplex* cuda_result, 
	double* cuda_cmi, double parameter,
	float* coff, double* s_2, int atom_nums, 
	double* kx, double* ky, int Height, int Width, int VOL)
{
	int id = (blockIdx.y * gridDim.x + blockIdx.x) * (blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x + threadIdx.x);
	...
	if(atom < atom_nums)
	{
		if (row < Height && col < Width)
		{
			float part1 = coff[atom * 8 + 3];

			int index = (int)coff[atom * 8 + 5] - 1;
			double* cmi_p = cuda_cmi + index * VOL;
			float part2 = (float)cmi_p[row * Width + col] * coff[atom * 8 + 4];

			index = (int)coff[atom * 8 + 7] - 1;
			cmi_p = cuda_cmi + index * VOL;
			float part3 = (float)cmi_p[row * Width + col] * coff[atom * 8 + 6];

			float part4 = expf((-1) * coff[atom * 8 + 2] * s_2[row * Width + col]);

			float part5 = (-2) * (coff[atom * 8 + 0] * kx[row * Width + col] + coff[atom * 8 + 1] * ky[row * Width + col]
			) * 3.14159265;

			float t1 = parameter * cosf(part5) * part1 * (part2 + part3) * part4;
			float t2 = parameter * sinf(part5) * part1 * (part2 + part3) * part4;
			cuda_result[row * Width + col].x = cuda_result[row * Width + col].x + t1;
			cuda_result[row * Width + col].y = cuda_result[row * Width + col].y + t2;
		}
	}
...
void p_corr(cufftComplex* p_PitchedPtr,int p_height,int p_width, size_t pitch2, int slices,
			double* ele_n_corr, double* ele_n_i_corr, 
			double* series_n_corr, double* series_n_i_corr, double* corr_info_matrix, int cim_len,
			double parameter, double* cuda_kx, double* cuda_ky, double* cuda_s_2, dim3 blockSize_P)
{
	...
	int x, y;
	double* corr_info_matrix_cuda;
	cudaMalloc((void**)&corr_info_matrix_cuda, sizeof(double)* p_vol * cim_len);
	cudaMemcpy(corr_info_matrix_cuda, corr_info_matrix, sizeof(double) * p_vol * cim_len, cudaMemcpyHostToDevice);
	...
	for (int slice = 0; slice < slices; slice++)
	{
		...
		rowHead = (cufftComplex *)((char *)p_PitchedPtr + pitch2 * slice);
		// set_Zero << < blockSize_P, threadsPerBlock >> >(rowHead, p_height, p_width);
		// 原子性
		if (series_n_corr != NULL && series_n_corr[slice] > 0)
		{
			int atom_nums = series_n_corr[slice];
			int len = 5 * atom_nums;
			...
			{
				for (int n = 0; n < 5; n++)
					coffMat[m * 5 + n] = ele_n_corr[sumSeries_n * 5 + m * 5 + n];
			}
			...
			for(int i = 0;i < ci; i++)
			{
				...
				if(batch_atom > 0)
				{
					...
					dim3 grid(x, y);
					cal_atomORion_fun_corr<<<grid, threadsPerBlock>>>(
						rowHead, corr_info_matrix_cuda, parameter, coffMat_cuda + start_atom * 5,  
						cuda_s_2, batch_atom, cuda_kx, cuda_ky, 
		...
		if (series_n_i_corr != NULL && series_n_i_corr[slice]>0)
		{
			int atom_nums_i = series_n_i_corr[slice];
			int len2 = 8 * atom_nums_i;
			coffAbsob = (float *)malloc(sizeof(float) * len2);
				//一次获取到当前传播层计算原子性和离子性矩阵的所有参数
			for (int m = 0; m < atom_nums_i; m++)
			{
				for (int n = 0; n < 8; n++)
					coffAbsob[m * 8 + n] = ele_n_i_corr[sumSeries_n_i * 8 + m * 8 + n];
			}
			sumSeries_n_i += atom_nums_i;
			...
			int ci = atom_nums_i / p_batchs + 1;
			for(int i = 0; i < ci; i++)
			{
				int start_atom = i * p_batchs;
				int end_atom = (i + 1) * p_batchs;
				if(end_atom > atom_nums_i)
				{
					end_atom = atom_nums_i;
				}
				int batch_atom = end_atom - start_atom;
				if(batch_atom > 0)
				{
					setThreadGrid(batch_atom, p_vol, &x, &y);
					dim3 grid(x, y);
					cal_atomORion_fun_i_corr<<<grid, threadsPerBlock>>>(
						rowHead, corr_info_matrix_cuda, parameter, coffAbsob_cuda + start_atom * 8,  
						cuda_s_2, batch_atom, cuda_kx, cuda_ky, 
						p_height, p_width, p_vol);
					cudaDeviceSynchronize();
				}
	...
	if (corr_info_matrix_cuda != NULL)
	{
		cudaFree(corr_info_matrix_cuda);
	}
	//printf_2d(p_PitchedPtr, p_height, p_width, slices, pitch2);
}
...
// 计算所有的p
void cal_p(double* atom_slice, int slice, double* absorb_n, int* sumSeries_n, 
			double* proj_coff_mat, dim3 blockSize_P,
           ...
           double parameter, int *allAtoms, int *allAtomsi)
{
	...
	*allAtoms += atom_nums;
	...
	if (atom_nums > 0)
	{
		...
		if (absorb_n == NULL)
		{
			int len = 13 * atom_nums;
			coffMat = (float *)malloc(sizeof(float) * len);
			//一次获取到当前传播层计算原子性和离子性矩阵的所有参数
			for (int m = 0; m < atom_nums; m++)
			{
				for (int n = 0; n < 13; n++)
					coffMat[m * 13 + n] = proj_coff_mat[*sumSeries_n * 13 + m * 13 + n];
			}
			cudaMalloc((void**)&coffMat_cuda, sizeof(float) * len);
			cudaMemcpy(coffMat_cuda, coffMat, sizeof(float) * len, cudaMemcpyHostToDevice);
			...
			for(int i = 0;i < ci; i++)
			{
				...
				if(batch_atom > 0)
				{
					setThreadGrid(batch_atom, p_vol, &x, &y);
					dim3 grid(x, y);
					cal_atomORion_fun << < grid, threadsPerBlock >> >(cuda_result1, coffMat_cuda+start_atom*13, cuda_s_2, batch_atom
		...
		else
		{
			...
			for(int i = 0;i < ci; i++)
			{
				...
				if(batch_atom>0)
				{
					...
					cal_absorb_fun << <grid, threadsPerBlock >> >(cuda_result_absorb, coffMat_cuda + start_atom * 13, coffAbsob_cuda + start_atom * 10
	...
	*sumSeries_n += atom_nums;
	...
	if (atomi_nums > 0)
	{
		...
		if (absorb_n_i == NULL)
		{
			...
			//计算该层系数所需要的空间长度
			int len = 25 * atomi_nums;
			coffMat = (float *)malloc(sizeof(float) * len);
			//一次获取到当前传播层计算原子性和离子性矩阵的所有参数
			for (int m = 0; m < atomi_nums; m++)
			{
				for (int n = 0; n < 25; n++)
					coffMat[m * 25 + n] = proj_coff_mat_i[*sumSeries_n_i * 25 + m * 25 + n];
			}
			cudaMalloc((void**)&coffMat_cuda, sizeof(float) * len);
			cudaMemcpy(coffMat_cuda, coffMat, sizeof(float) * len, cudaMemcpyHostToDevice);
			...
			for(int i = 0;i < ci; i++)
			{
				...
				if(batch_atom>0)
				{
					setThreadGrid(batch_atom, p_vol, &x, &y);
					dim3 grid(x, y);
					cal_atomAndion_fun << <grid, threadsPerBlock >> >(cuda_result2, coffMat_cuda + start_atom * 25, cuda_s_2, batch_atom
																		, cuda_kx, cuda_ky, p_height, p_width, p_vol);
																		cudaDeviceSynchronize();
				}
			}
			...
			for(int i = 0;i < ci; i++)
			{
				...
				if(batch_atom>0)
				{
					...
					cal_atomAndion_fun << <grid, threadsPerBlock >> >(
							cuda_result2, coffMat_cuda + start_atom * 25, cuda_s_2, batch_atom
						, cuda_kx, cuda_ky
						, p_height, p_width, p_vol);
						cudaDeviceSynchronize();
					cal_absorb_fun2 << <grid, threadsPerBlock >> >(
						cuda_result_absorb, coffMat_cuda + start_atom * 25, coffAbsob_cuda + start_atom * 10, cuda_s_2, batch_atom
						, cuda_kx, cuda_ky
						, p_height, p_width, p_vol);	
						cudaDeviceSynchronize();
				}
	...
	*sumSeries_n_i += atomi_nums;
	...
	if (atom_nums != 0 || atomi_nums != 0)
	{
		...
		cufftComplex* rowHead;
		rowHead = (cufftComplex *)((char *)p_PitchedPtr + pitch2 * (slice - 1));
		copy_result_to_p << <blockSize_P, threadsPerBlock >> >(cuda_result1, rowHead, p_height, p_width,
																parameter);
	}
}
...
*/
	...
	{
		printf("corr running\n");
		p_corr( p_PitchedPtr, p_height, p_width, pitch2, slices,
				ele_n_corr, ele_n_i_corr, 
				series_n_corr,  series_n_i_corr,  corr_info_matrix, cim_len,
				parameter, cuda_kx, cuda_ky,  cuda_s_2,  blockSize_P);
		if (series_n_i_corr == NULL){
			printf("series_n_i_corr null\n");
		}
	}
